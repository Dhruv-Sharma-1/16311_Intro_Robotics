...
int main() {
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    if (fabs(x - check[0]) < 0.25 && fabs(y - check[1]) < 0.25 && fabs(z - check[2]) < 0.25)
      {
        if (!pcheck) {
          // then save the check point picture 
          const unsigned char *image = wb_camera_get_image(cam);
          const char *filename = "/Users/dhruvsharma/Desktop/16311 - Intro to Robotics/Lab/10 Teleop/checkpoint.png";

          int saved = wb_camera_save_image(cam, filename, 100);

          if (saved == 0)
            // mark that checkpoint has been reached
            printf("Checkpoint reached at: ");
            printf("x = %f, y = %f, z = %f\n", x, y, z);
            pcheck = true;
    ...
    if (fabs(x - xS) < 0.4 && fabs(y - yS) < 0.4 && fabs(z - zS) < 0.4)
      {
